...
!function () {
  ...
  // ===========================================================================
  // SHARE CARD DATA STORES
  // ===========================================================================

  /** Original values for share card PNL elements (element -> original number) */
  const originalShareCardPnlValues = new Map();

  /** Tracks which share card PNL elements have been modified */
  const modifiedShareCardPnlElements = new WeakSet();

  /** Tracks share card PNL elements with protection MutationObserver */
  const shareCardPnlProtectedElements = new WeakSet();

  /** Timestamps of when each share card element was modified */
  const shareCardModificationTimes = new Map();

  /** Maps share card container to modification state (for avoiding duplicate processing) */
  const processedShareCards = new WeakSet();
  ...
  const SELECTORS = {
    ...
    // ---- Share Card elements (position share dialog/modal) ----
    shareCardContainers: [
      "#share-position-poster",
      "#web-share-sdk-share-poster .PosterImage",
      'div[id="share-position-poster"]'
    ],

    shareCardPnlParents: [
      "#share-position-poster .mb-\\[16px\\].text-Sell",
      "#share-position-poster .mb-\\[16px\\].text-Buy",
      '#share-position-poster div[class*="mb-[16px]"][class*="text-Sell"]',
      '#share-position-poster div[class*="mb-[16px]"][class*="text-Buy"]'
    ],

    shareCardPnlElements: [
      "#share-position-poster .t-headline1.\\!leading-none",
      "#share-position-poster .mb-\\[16px\\] > .t-headline1",
      "#share-position-poster .text-Sell .t-headline1",
      "#share-position-poster .text-Buy .t-headline1"
    ],

    shareCardRoiElements: [
      "#share-position-poster .mb-\\[16px\\] > .typography-subtitle4",
      "#share-position-poster .text-Sell .typography-subtitle4",
      "#share-position-poster .text-Buy .typography-subtitle4"
    ],
  ...
  function updatePnlFromRoi(card) {
    try {
      ...
      const sign = multipliedPnl > 0 ? "+" : multipliedPnl < 0 ? "-" : "";
      ...
      if (pnl.textContent !== formattedPnl) {
        ...
        // Validation checkpoint: Verify negative values have minus sign
        if (calculatedPnl < 0 && !formattedPnl.startsWith("-")) {
          console.error("PNL Modifier: CRITICAL - Negative PNL missing minus sign!", {
            calculatedPnl,
            formattedPnl,
            sign
          });
        ...
        // Clear flag synchronously to eliminate race condition
  ...
  // ===========================================================================
  // SHARE CARD PROCESSING FUNCTIONS
  // ===========================================================================

  /**
   * Process a SHARE CARD PNL element.
   * Share cards display PNL in .t-headline1 element with a separate USDT span.
   * CRITICAL: Must preserve the USDT span child element.
   * 
   * Expected format: 
   *   - Text node: "+0.26" or "-0.09"
   *   - Span child: "USDT" (must be preserved)
   * Result: "+2,600.XX" with preserved USDT span
   *
   * @param {Element} pnlElement - The .t-headline1 element containing PNL
   */
  function processShareCardPnlElement(pnlElement) {
    try {
      if (!document.body.contains(pnlElement)) return;

      // Extract current text from TEXT NODE only (exclude the USDT span)
      let currentText = "";
      for (let node of pnlElement.childNodes) {
      ...
      if (originalShareCardPnlValues.has(pnlElement)) {
        originalValue = originalShareCardPnlValues.get(pnlElement);
      } else {
        originalValue = parseFloat(match[1].replace(/,/g, ""));
        
        // Skip if too small (< 0.01)
        if (Math.abs(originalValue) < 0.01) {
          console.log("PNL Modifier: Share card PNL too small to modify:", originalValue);
          return;
        }
        
        originalShareCardPnlValues.set(pnlElement, originalValue);
        pnlElement.dataset.originalSharePnl = originalValue.toString();
        pnlElement.dataset.originalText = currentText;

        console.log("PNL Modifier: Found share card PNL element:", {
          text: currentText,
          originalValue: originalValue,
          classes: pnlElement.className
        });
      }
      ...
      for (let node of pnlElement.childNodes) {
        if (node.nodeType === 3) { // TEXT_NODE
          if (node.nodeValue.trim() !== formattedText) {
      ...
      if (textNodeUpdated) {
        modifiedShareCardPnlElements.add(pnlElement);
        shareCardPnlProtectedElements.add(pnlElement);
        shareCardModificationTimes.set(pnlElement, Date.now());
        setupShareCardPnlProtection(pnlElement, formattedText);

        console.log("PNL Modifier: Updated share card PNL:", {
          original: currentText,
          modified: formattedText,
          multiplier: MULTIPLIER
        });
      ...
      console.warn("PNL Modifier: Error processing share card PNL:", err);
    }
  }

  /**
   * Queue a share card for processing.
   * Share cards are ephemeral (appear/disappear), process immediately.
   * @param {Element} element - The share card PNL element
   */
  function queueShareCardUpdate(element) {
    // Process immediately with microtask delay for rendering
    Promise.resolve().then(() => {
      if (document.body.contains(element)) {
        processShareCardPnlElement(element);
      }
    });
  }

  /**
   * Attach a protection observer to a SHARE CARD PNL element.
   * Watches for text node changes while preserving the USDT span child.
   *
   * @param {Element} element - The .t-headline1 share card PNL element
   * @param {string} expectedText - The text we set in the text node (e.g., "+2,600.12")
   */
  function setupShareCardPnlProtection(element, expectedText) {
    ...
    const observer = new MutationObserver((mutations) => {
      ...
      mutations.forEach((mutation) => {
        ...
        try {
          // Extract current text from text nodes only
          let currentText = "";
          ...
          if (Math.abs(currentNumber - expectedNumber) > 0.01) {
            // Check if this is a genuine new value
            const storedOriginal = originalShareCardPnlValues.get(element);
            if (storedOriginal && isValidPnlChange(currentNumber, storedOriginal)) {
              // Genuine update: re-multiply
              originalShareCardPnlValues.set(element, currentNumber);
              const newMultiplied = currentNumber * MULTIPLIER;
              const newSign = newMultiplied > 0 ? "+" : newMultiplied < 0 ? "-" : "";
              const newText = `${newSign}${formatNumber(Math.abs(newMultiplied), 'pnl')}`;
              ...
              element._shareCardProtectionObserver && element._shareCardProtectionObserver.disconnect();
              setupShareCardPnlProtection(element, newText);
              return;
            }
            ...
            for (let node of element.childNodes) {
              if (node.nodeType === 3) {
                node.nodeValue = expectedText;
              }
            }
          }
        } catch (err) {
          console.warn("PNL Modifier: Error in share card PNL protection:", err);
        }
      });
    });
    ...
    element._shareCardProtectionObserver = observer;

    // Auto-disconnect after 60 seconds (share cards are short-lived)
    setTimeout(() => {
      if (!document.body.contains(element)) {
        observer.disconnect();
      }
  ...
  function performFullScan() {
    try {
      ...
      // --- Share Card PNL elements (ephemeral share dialogs) ---
      SELECTORS.shareCardContainers.forEach((selector) => {
        try {
          document.querySelectorAll(selector).forEach((shareCard) => {
            if (isInViewport(shareCard)) {
              // Find PNL element within share card
              SELECTORS.shareCardPnlElements.forEach((pnlSelector) => {
                ...
                if (pnlEl && !modifiedShareCardPnlElements.has(pnlEl)) {
                  ...
                  console.log("PNL Modifier: Processing share card PNL in full scan");
                }
              });
            }
          });
  ...
  function initialize() {
    try {
      ...
      setInterval(() => {
        ...
        // --- Verify and re-apply SHARE CARD PNL modifications ---
        SELECTORS.shareCardContainers.forEach((selector) => {
          try {
            document.querySelectorAll(selector).forEach((shareCard) => {
              SELECTORS.shareCardPnlElements.forEach((pnlSelector) => {
                const element = shareCard.querySelector(pnlSelector);
                if (element && originalShareCardPnlValues.has(element)) {
                  // Extract current text from text nodes
                  let currentText = "";
                  for (let node of element.childNodes) {
                    if (node.nodeType === 3) {
                      currentText += node.nodeValue;
                  ...
                  if (match && match[1]) {
                    ...
                    if (Math.abs(currentNumber - expectedMultiplied) > 0.01) {
                      if (isValidPnlChange(currentNumber, storedOriginal)) {
                        ...
                        if (!shareCardPnlProtectedElements.has(element)) {
                          setupShareCardPnlProtection(element, restoredText);
                        }
                      }
                    }
                  }
        ...
        // --- Clean up detached share card elements ---
        for (const [element, timestamp] of shareCardModificationTimes.entries()) {
          if (!document.body.contains(element) || now - timestamp > 60000) {
            shareCardModificationTimes.delete(element);
            originalShareCardPnlValues.delete(element);
          }
        }
