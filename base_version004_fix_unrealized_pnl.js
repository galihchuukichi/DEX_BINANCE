...
!function () {
  ...
  function updatePnlFromRoi(card) {
    try {
      ...
      // Extract current ROI value from dataset or parse from text
      let roiValue;
      if (roi.dataset.originalRoi) {
        roiValue = parseFloat(roi.dataset.originalRoi);
      } else {
      ...
      if (roiValue === null || isNaN(roiValue)) return;

      // Get original margin value from dataset (NEVER parse displayed text)
      let originalMargin;
      if (margin.dataset.originalMargin) {
        originalMargin = parseFloat(margin.dataset.originalMargin);
      } else if (originalMobileMarginValues.has(margin)) {
      ...
      if (pnl.textContent !== formattedPnl) {
        ...
        console.log("PNL Modifier: Updated PNL from ROI:", {
   ...
   * When ROI changes, recalculate PNL immediately.
   *
