/**
 * =============================================================================
 * PNL (Profit & Loss) Modifier for Binance Mobile Interface
 * =============================================================================
 *
 * This script modifies displayed PNL, position size, and margin values on the
 * Binance mobile trading interface. It multiplies the original values by a
 * configurable multiplier (default: 10,000x), replaces the displayed text, and
 * protects the modified values from being overwritten by Binance's DOM updates.
 *
 * Key features:
 *   - Mobile-only interface support (position cards with .py-[16px])
 *   - Real-time PNL calculation from ROI × Margin formula
 *   - macOS and Windows-specific timing optimizations
 ...
 *   - Handles mobile PNL, Size, and Margin elements in 3-column grid layout
 *
 * The script generates independent random decimal places to simulate
 * realistic-looking fractional values after multiplication, eliminating
...
!function () {
  ...
  /** Tracks which mobile PNL elements have been modified with new PNL text */
  const modifiedPnlElements = new WeakSet();

  /** Timestamps of when each mobile PNL element was last modified */
  const pnlModificationTimes = new Map();

  /** Tracks mobile PNL elements that have a protection MutationObserver attached */
   ...
   * Each element type (mobile PNL, mobile size, mobile margin,
  ...
  /**
   * Collection of CSS selectors used to find mobile PNL, size, margin,
   * and container elements in the Binance UI.
   */
  const SELECTORS = {
   ...
   * Handles the mobile format (no "USDT" suffix, just the number).
   *
  ...
  function handleMutations(mutations) {
    ...
    try {
      for (const mutation of mutations) {
        ...
        // =================================================================
        // CASE 2: TEXT CHANGES — Mobile PNL elements
        // =================================================================
        if (mutation.type === "characterData" || mutation.type === "childList") {
        ...
        // =================================================================
        // CASE 3: TEXT CHANGES — Mobile SIZE elements
        // =================================================================
        if (mutation.type === "characterData" || mutation.type === "childList") {
          let targetEl = mutation.target;
          if (targetEl.nodeType !== 1) targetEl = targetEl.parentElement;
        ...
        // =================================================================
        // CASE 4: TEXT CHANGES — ROI elements (triggers PNL recalculation)
        // =================================================================
        if (mutation.type === "characterData" || mutation.type === "childList") {
          let targetEl = mutation.target;
          if (targetEl.nodeType !== 1) targetEl = targetEl.parentElement;
  ...
  /**
   * Perform a full scan of the visible DOM for mobile PNL, size, and margin elements.
  ...
  function performFullScan() {
    try {
      // ==================================================================
      // STEP 1: Link position cards for ROI-based PNL calculation
      // ==================================================================
      SELECTORS.positionCards.forEach((selector) => {
        try {
      ...
      // ==================================================================
      // STEP 3: Mobile MARGIN elements (middle column in grid-cols-3)
      // ==================================================================
      SELECTORS.mobileMarginElements.forEach((selector) => {
        try {
          document.querySelectorAll(selector).forEach((element) => {
      ...
      console.warn("PNL Modifier: Error in performFullScan:", err);
    }
  }
  ...
  function initialize() {
    try {
      ...
      setInterval(() => {
        ...
        for (const [element, timestamp] of mobileMarginModificationTimes.entries()) {
          if (now - timestamp > CACHE_EXPIRY_MS) {
            mobileMarginModificationTimes.delete(element);
            originalMobileMarginValues.delete(element);
          }
        }
        ...
        SELECTORS.mobilePnlElements.forEach((selector) => {
          try {
            document.querySelectorAll(selector).forEach((element) => {
              if (element.classList.contains("text-TextSell") || element.classList.contains("text-TextBuy")) {
                // Skip elements with ROI-based PNL (they're updated by ROI changes)
                if (pnlToCardMapping.has(element)) return;
                
                const match = element.textContent.trim().match(/^([+-]?[\d,]+\.?\d*)$/);
                if (match && match[1]) {
                  const currentNumber = parseFloat(match[1].replace(/,/g, ""));
                  // Note: Mobile PNL without ROI doesn't use stored original values
                  // It's re-processed from scratch each time
                }
              }
            });
          } catch (e) { /* selector may fail */ }
  ...
  /**
   * Poll the DOM until Binance mobile UI elements are detected, then initialize.
   * Checks for:
   *   - Mobile position card containers (.py-[16px])
  ...
  function waitForDomAndStart() {
    ...
    const pollingInterval = setInterval(() => {
      ...
      try {
        // Check if mobile position cards exist
        const hasMobilePositions =
          document.querySelectorAll('.py-\\[16px\\]').length > 0 ||
        ...
        if (hasMobilePositions || hasContainer || hasBinanceUI || attempts >= 50) {
          clearInterval(pollingInterval);
          initialize();

          if (hasMobilePositions || hasContainer) {
